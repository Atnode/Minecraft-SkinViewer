...
function GetBase64(url, success, error, extra) {
    $.ajax({
        url: '../base64/base64.php',
        data: {
            url: url
        },
        dataType: 'jsonp',
        jsonp: 'callback',
        jsonpCallback: 'Base64convert',
        cache: true,
        error: error,
        timeout: 5000
    });

    Base64convert = function(data) {
        try {
            if (!data.url || !data.image || !data.time) {
                error();
            } else {
                var image = new Image();
...
function play() {
    if (animating = !animating) {
        ...
        animLoop(function(delta, now) {
            changePosition({
                left_arm: {
                    x: Math.cos(now / 250) * 60 - 45,
                    z: -15 + Math.sin(now / 500) * 10
                },
                right_arm: {
                    x: Math.sin(now / 500) * 30,
                    z: 15 - Math.sin(now / 500) * 10
                },
                left_leg: {
                    x: Math.cos(now / 250) * 45,
                    z: -5 + Math.sin(now / 500) * 5
                },
                right_leg: {
                    x: -Math.cos(now / 250) * 45,
                    z: 5 - Math.sin(now / 500) * 5
...
function generateNewRandomPosition() {
    centers.left_leg.index = 7;
    centers.left_leg.point = 1;
    centers.right_leg.index = 11;
    centers.right_leg.point = 2;
    ...
    if (Math.random() > 0.6) positions.random.head = {
        x: 0,
        y: 0,
        z: 0
    };
...
function bend(rot) {
    ...
    var saved_rot = {
        x: accumulativeRotation.x,
        y: accumulativeRotation.y
    };
    updateHolding();
    rotatePlayer(Math.PI / 2, 0);

    if (player.left_arm$hold)
        for (var side in player.left_arm$hold) player.left_arm$hold[side].forEach(function(s, i) {
    ...
    if (rot.left_arm_r1 || rot.left_arm_r2) {
        for (var side in player.left_arm) player.left_arm[side].forEach(function(s, i) {
            s.points.forEach(function(p, j) {
                var a = rot.left_arm_r1 * Math.PI,
                    d = Math.PI * rot.left_arm_r2,
                    s = 3,
                    h = -p[1],
                    r = h;
                r -= 6;
                r = Math.max(0, Math.min(12, r)) / 12;
                h -= 5;
                h = Math.max(0, Math.min(s, h));
    ...
    if (player.right_arm$hold)
        for (var side in player.right_arm$hold) player.right_arm$hold[side].forEach(function(s, i) {
            s.points.forEach(function(p, j) {
    ...
    if (rot.right_arm_r1 || rot.right_arm_r2) {
        for (var side in player.right_arm) player.right_arm[side].forEach(function(s, i) {
            s.points.forEach(function(p, j) {
                var a = rot.right_arm_r1 * Math.PI,
                    d = Math.PI * rot.right_arm_r2,
                    s = 3,
                    h = -p[1],
                    r = h;
                r -= 6;
    ...
    if (rot.right_leg_r1)
        for (var side in player.right_leg) player.right_leg[side].forEach(function(s, i) {
            s.points.forEach(function(p, j) {
                var a = rot.right_leg_r1 * -Math.PI,
                    s = 4,
                    h = -p[1],
                    r = (h - 12) / 12;
                h -= 16;
